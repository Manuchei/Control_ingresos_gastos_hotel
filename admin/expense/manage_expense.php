...
<script>
	...
	$(document).ready(function(){
        $('.select2').select2({placeholder:"Por favor seleccione aquí el",width:"relative"})
        $('.number').on('load input change',function(){
            var txt = $(this).val()
                ...
                if(txt.slice(-1) == '.'){
                    ...
                    ntxt += "."+txt[1]
                    ntxt = ntxt > 0 ? ntxt : 0;
                    txt = parseFloat(ntxt).toLocaleString('es-ES')
                }
                $(this).val(txt)
        })
		...
		$('#expense-form').submit(function(e){
            ...
            if(parseFloat(amount) > parseFloat(cat_balance)){
                var el = $('<div>')
                    el.addClass("alert alert-danger err-msg mt-2").text("La cantidad introducida es mayor que el balance de la categoría seleccionada.")
