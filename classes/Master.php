...
Class Master extends DBConnection {
	...
	function save_category(){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Nueva categoría agregada correctamente.");
			else
	...
	function save_budget(){
		...
		if($save){
			...
			if($update_balance == 1){
				$resp['status'] ='success';
				$this->settings->set_flashdata('success'," Presupuesto guardado.");
			}else{
				$resp['status'] = 'failed';
	...
	function delete_budget(){
		...
		if($del){
			...
			if($update_balance == 1){
				$resp['status'] ='success';
				$this->settings->set_flashdata('success',"Presupuesto eliminado.");
			}else{
				$resp['status'] = 'failed';
				$resp['error'] = $update_balance;
			}
	...
	function save_expense(){
		...
		if($save){
			...
			if($update_balance == 1){
				$resp['status'] ='success';
				$this->settings->set_flashdata('success'," Gasto guardado correctamente.");
			}else{
				$resp['status'] = 'failed';
				$resp['error'] = $update_balance;
			}
	...
	function delete_expense(){
		...
		if($del){
			...
			if($update_balance == 1){
				$resp['status'] ='success';
				$this->settings->set_flashdata('success',"Gasto eliminado correctamente.");
			}else{
				$resp['status'] = 'failed';
				$resp['error'] = $update_balance;
			}
