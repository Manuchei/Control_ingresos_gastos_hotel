...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				$this->settings->set_flashdata('success','Detalles de usuario guardados correctamente.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
			...
			if($qry){
				$this->settings->set_flashdata('success','Detalles de usuario actualizados correctamente.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
						$this->settings->set_userdata($k,$v);
	...
	public function delete_users(){
		...
		if($qry){
			$this->settings->set_flashdata('success','Detalles de usuario eliminados correctamente.');
			return 1;
		}else{
			return false;
		}
